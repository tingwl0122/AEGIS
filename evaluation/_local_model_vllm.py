...
def calculate_metrics(results: List[Dict]) -> Dict:
    ...
    parse_fail = sum(1 for r in results if ("model_detection" in r) and (not r.get("parse_ok", True)))
    return {
        "total_samples": total,
...
SYSTEM_MESSAGE = "You are a precise analyst. Please follow the instructions carefully and provide detailed reasoning before giving your final JSON answer."
...
# Response parser
# ============================================================
def parse_response(response_text: str) -> tuple[Dict, bool]:
    """
    Parse model response into JSON format.
    Returns: (parsed_dict, parse_ok)
      - parse_ok=False means we failed and returned empty faulty_agents.
    """
    if not response_text or not response_text.strip():
        return {"faulty_agents": []}, False
    ...
    if m:
        try:
            return {"faulty_agents": json.loads(m.group(1))}, True
        except json.JSONDecodeError:
            pass
    ...
    agent_matches = re.findall(
        r'"agent_name"\s*:\s*"([^"]+)"[^}]*"error_type"\s*:\s*"([^"]+)"',
        clean
    )
    if agent_matches:
        agents = [{"agent_name": n, "error_type": t} for n, t in agent_matches]
        return {"faulty_agents": agents}, True
...
def _dp_worker(
    ...
    try:
        ...
        engine_kwargs = dict(
            ...
            max_lora_rank=512 if lora_path else None,
        ...
        # we use greedy decoding for answering
        ...
        num_parse_fail = sum(1 for _, r in results if not r.get("parse_ok", True))
        worker_logger.info(f"Worker {worker_idx} parse failures: {num_parse_fail}/{len(results)}")
    ...
    def _detect_with_vllm(self, batch_messages: List[List[Dict]]) -> tuple[List[Dict], List[str]]:
        ...
        try:
            ...
            for output in outputs:
                ...
                parsed, ok = parse_response(text)
                parsed["_parse_ok"] = ok   # embed the flag (or return separately)
...
def main():
    ...
    if Path(args.output).exists():
        ...
        num_parse_fail = sum(
            ...
            if ("model_detection" in r) and (not r.get("parse_ok", True))
        )
        logger.info(f"  Parse failures:         {num_parse_fail}")
        ...
        logger.info(f"  Parse failures:         {metrics['parse_failures']}")
        logger.info("=" * 60)
